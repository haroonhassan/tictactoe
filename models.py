...
class TicTacToeNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i in range(config.model.num_hidden_layers):
            if i == 0:
                ...
                layers.append(nn.Linear(config.model.hidden_size, config.model.hidden_size))
    ...
    def _build_policy_head(self, config) -> nn.Module:
        ...
        return nn.Sequential(
            nn.Linear(config.model.hidden_size, config.model.output_policy_size)
        )
    
    def _build_value_head(self, config) -> nn.Module:
        """Build the value output head"""
        return nn.Sequential(
            nn.Linear(config.model.hidden_size, config.model.output_value_size),
    ...
    def get_architecture_summary(self) -> str:
        ...
        summary.append(f"  Hidden layers: {self.config.model.num_hidden_layers}")
        summary.append(f"  Hidden size: {self.config.model.hidden_size}")
        summary.append(f"  Activation: {self.config.model.activation}")
        summary.append(f"  Output policy size: {self.config.model.output_policy_size}")
        summary.append(f"  Output value size: {self.config.model.output_value_size}")
...
class ResNetTicTacToe(TicTacToeNet):
    ...
    def __init__(self, config):
        ...
        # Initial projection
        layers.append(nn.Linear(config.model.input_size, config.model.hidden_size))
        layers.append(nn.BatchNorm1d(config.model.hidden_size))
        layers.append(nn.ReLU())
        
        # Residual blocks
        for _ in range(config.model.num_hidden_layers):
            layers.append(ResidualBlock(config.model.hidden_size, config.model.activation))
